...
const SingleCard = ({data}) => {
    ...
    useEffect(() => {
        const fetchStarships = async () => {
            try {
                const response = await Promise.all(
                    starships.map(url => fetch(`${url}?format=json`, { referrerPolicy: "unsafe-url"}).then(res => res.json()))
                );
        ...
        const fetchFilms = async () => {
            try {
                const response = await Promise.all(
                    planets.map(url => fetch(`${url}?format=json`, { referrerPolicy: "unsafe-url"}).then(res => res.json()))
                );
                setPlanets(response);
                setLoading(false);
            } catch (error) {
